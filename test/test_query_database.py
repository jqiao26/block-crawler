...
def setup_teardown_db(db_path):
    db = Database(db_path)
    blocks = [
        Block("block1_hash", "1", "0x659207ff", ["transaction1"]),
        Block("block2_hash", "2", "0x6592080b", ["transaction2"]),
        Block("block3_hash", "3", "0x65920878", ["transaction3", "transaction4"]),
    ]
    db._save_block_data_batch(blocks)
    ...
    for transaction in transactions:
        db._save_transaction_data(transaction)
    yield
